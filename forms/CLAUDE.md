...
- 数値入力は Input が string を返すため、**`components/form/numeric-input.tsx` の `NumericInput` を使う** (string ↔ number 変換とローカル text state を内包)。空入力時は `null` を返すため、スキーマ側は `z.number().nullable().optional()` にする (`.optional()` のみだと `useFieldArray` 内で `undefined` を渡したときに RHF が初期値に巻き戻すバグがある)。`z.preprocess` は `z.infer` の入力型が `unknown` になり `defaultValues` と型衝突するため避ける
