...
- `app/(tabs)/` — メインタブ (index / lesson / equipment / purchase-plan / settings)。認証済み時に遷移
...
- **練習記録は `(user_id, practiced_at)` で同日 1 件のみ**: DB 側に UNIQUE 制約 `practice_sessions_user_id_practiced_at_key`、クライアント側は `app/practice-log-form.tsx` で URL `id` (= `urlId`) と内部 `effectiveId` を併存させ、フォーム表示時 / 日付欄変更時に同日既存セッションを検出して透過的に編集モードへ切替える。新機能で別経路から `practice_sessions` へ INSERT を増やす場合はこの不変条件を踏まえる
