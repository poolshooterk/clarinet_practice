...
Expo アプリ開発の**汎用テンプレート**として継続的に維持・整備するリポジトリ。新機能や依存関係を追加する際は、プロジェクト固有の要件よりも品質と再利用性を優先すること。
...
expo-router v6 のファイルベースルーティング。`app/_layout.tsx` が `<TamaguiProvider>` で `<Stack />` をラップ、`useColorScheme` で OS テーマに追従する。
...
| 新しいルート          | `app/<route>.tsx` (`typedRoutes` 有効)                                                  |
...
- 参照実装: `components/profile-form.tsx` + `forms/profile.ts` + `components/form/field-error.tsx`。`app/index.tsx` からは `<ProfileForm />` を貼るだけにする (page と form の責務分離)
...
- 参照実装: `store/counter.ts` (非永続) / `store/settings.ts` (永続化) と `app/index.tsx` の Zustand demo セクション
