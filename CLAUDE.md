...
- `app/(tabs)/` — メインタブ (index / lesson / equipment / purchase-plan / annual-goals / settings の順)。認証済み時に遷移
...
- **録音再生のシークは duration 判明後にだけ有効化する**: `RecordingCard` の再生位置バーは Tamagui `Slider` で、`disabled={duration === 0}` により**一度再生して Sound が読み込まれた (= `durationMillis` が来た) 後**にだけドラッグ可能になる。Sound は再生ボタン押下時に遅延生成する現状の方針を維持し (録音中の `Audio.setAudioModeAsync` タイミングを乱して無音化する回帰を避けるため)、再生前のスクラブは意図的に不可。ドラッグ中は `seeking` state と `seekingRef` (ステータスコールバック内で同期参照する ref) を立て、再生進行による `setPosition` を抑止して表示値の上書きを防ぐ。確定は `onSlideEnd` で `Sound.setPositionAsync(value)` (再生中は継続再生・一時停止中は位置のみ移動)。Tamagui `Slider` のドラッグジェスチャは host node に `onSlideEnd` 等が露出せずレイアウト計測も走らないため jest では再現不可 → unit/integration で検証するのは `aria-disabled` の切替と時刻ラベルまでで、実シーク動作は実機/E2E 確認 (DateTimePicker と同じ扱い)
