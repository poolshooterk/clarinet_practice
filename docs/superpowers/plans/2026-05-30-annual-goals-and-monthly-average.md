...
describe('MonthlyMilestoneForm', () => {
  ...
  it('正常入力で upsertMilestone が呼ばれる', async () => {
    ...
    fireEvent.press(getByText('保存'));
    await waitFor(() => {
      expect(upsertSpy).toHaveBeenCalledWith('g1', expect.objectContaining({ month: 5, text: 'X' }));
    });
  });
});
```
...
export default function AnnualGoalDetail() {
  ...
  const milestoneByMonth = useMemo(() => {
    const map = new Map<number, (typeof goal extends undefined ? never : NonNullable<typeof goal>)['milestones'][number]>();
...
{yearEndAvailable && (
  <Button
    size="$2"
    onPress={() =>
      router.push(`/monthly-milestone-form?goalId=${goal.id}&month=12&yearEnd=1`)
    }
    theme="blue"
  >
    12月マイルストーンを編集
