...
export function PurchasePlanForm() {
  ...
  const progressPct = plan && plan.targetPrice > 0
    ? Math.min(currentSavings / plan.targetPrice, 1)
    : 0;
      ...
      <YStack gap="$4" p="$4" pb="$8">
        ...
        <Card elevation="$2" borderWidth={1} borderColor="$borderColor" p="$4" gap="$3">
          ...
          <InstrumentPicker
            value={
              makerId
                ? { makerId, makerName, modelId, modelName }
                : null
            }
            onChange={(v) => {
...
describe('PurchasePlanForm (integration)', () => {
  beforeEach(() => {
    ...
    jest.clearAllMocks();
    (
      require('@/lib/supabase').supabase.auth.getUser as jest.Mock
